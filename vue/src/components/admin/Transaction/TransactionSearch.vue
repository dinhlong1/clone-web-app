          ...
          <table>
            ...
            <tbody>
              <tr v-for="transaction in transactions" :key="transaction">
                <td>{{ convertTime(transaction.created_at) }}</td>
                <td>{{ transaction.users.email }}</td>
                <td>{{ transaction.description }}</td>
                <td>{{ convertOption(transaction.option) }}</td>
                <td>{{ transaction.money }}</td>
                <th class="confirm">
                  {{ transaction.confirm ? "Thành Công" : "Thất Bại" }}
                </th>
              </tr>
            </tbody>
          </table>
...
import { changePage } from "../../../services/api/admin.js";
...
export default {
  ...
  methods: {
    async getData() {
      ...
      var url = `/api/v1/transaction/search/?${this.$route.params.query}&${this.$route.params.page}`;
      console.log(url);
    ...
    search(query) {
      this.$router.push(`/admin/transaction/search/query=${query}/page=1`);
    },
    createPgn() {
      ...
      function createPagination(totalPage, page, query) {
        ...
        if (page > 1 && page > 2) {
          ...
          liTag += `<a href="/admin/transaction/search/query=${query}/page=${page -
            1}/"><li class="btn prev"><span><i class="fas fa-angle-left"></i> Prev</span></li></a>`;
        }
        if (page > 1 && page == 2) {
          //show the next button if the page value is greater than 1
          liTag += `<a href="/admin/transaction/search/query=${query}/"><li class="btn prev"><span><i class="fas fa-angle-left"></i> Prev</span></li></a>`;
        }

        if (page > 2 && totalPage > 4) {
          //if page value is less than 2 then add 1 after the previous button
          liTag += `<a href="/admin/transaction/search/query=${query}/"><li class="first numb"><span>1</span></li></a>`;
        ...
        for (var plength = beforePage; plength <= afterPage; plength++) {
          ...
          liTag += `<a href="/admin/transaction/search/query=${query}/page=${plength}"><li class="numb ${active}"><span>${plength}</span></li></a>`;
        }
        ...
        if (page < totalPage - 1 && totalPage > 4) {
          ...
          liTag += `<a href="/admin/transaction/search/query=${query}/page=${totalPage}"><li class="last numb" ><span>${totalPage}</span></li></a>`;
        }
